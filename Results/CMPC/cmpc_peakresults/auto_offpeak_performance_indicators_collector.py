...
def run_performance_indicators(tripinfo_path, summary_path):
    ...
    output_lines.append("QUEUE LENGTHS  10AM-2PM - 15 MINUTE INTERVALS")
    output_lines.append(str([extract_summary_15min_ql(start, end)/24 for start, end in intervals]))
    output_lines.append("QUEUE TIMES  10AM-2PM - 15 MINUTE INTERVALS")
    output_lines.append(str([extract_tripinfo_15min_qt(start, end) for start, end in intervals]))
    output_lines.append("FLOW RATES  10AM-2PM - 15 MINUTE INTERVALS")
...
def main():
    sum_q_len_4hr = sum_q_time_4hr = sum_vfr_4hr = 0.0
    sum_q_len_whole = sum_q_time_whole = sum_vfr_whole = 0.0
    sum_cars_serviced = 0
    num_sims = 0

    # Accumulators for 15-min interval performance
    ...
    with open(OUTPUT_FILE, 'w') as out_file:
        for idx, subdir in enumerate(subdirs, start=1):
            print(idx, subdir)
            timestamp = subdir
            tripinfo_path = os.path.join(RESULTS_DIR, subdir, f"tripinfo.xml")
            summary_path = os.path.join(RESULTS_DIR, subdir, f"summary_out.xml")
            ...
            for line in results:
                if "(4 HR) Average queue length" in line:
                    sum_q_len_4hr += float(line.split(":")[1])
                elif "(4 HR) Average time in queue" in line:
                    sum_q_time_4hr += float(line.split(":")[1])
                elif "(4 HR) Average vehicular flow rate" in line:
                    sum_vfr_4hr += float(line.split(":")[1])
                elif "(WHOLE) Average queue length" in line:
                    sum_q_len_whole += float(line.split(":")[1])
                elif "(WHOLE) Average time in queue" in line:
                    sum_q_time_whole += float(line.split(":")[1])
                elif "(WHOLE) Average vehicular flow rate" in line:
                    sum_vfr_whole += float(line.split(":")[1])
                elif "Cars serviced" in line:
                    sum_cars_serviced += int(float(line.split(":")[1]))
            
            # --- Collect per-simulation interval data ---
            try:
                q_len_index = results.index("QUEUE LENGTHS  10AM-2PM - 15 MINUTE INTERVALS")
                q_time_index = results.index("QUEUE TIMES  10AM-2PM - 15 MINUTE INTERVALS")
                flow_rate_index = results.index("FLOW RATES  10AM-2PM - 15 MINUTE INTERVALS")
        ...
        if num_sims > 0:
            # --- Write average performance indicators results ---
            out_file.write("- - - AVERAGED PERFORMANCE INDICATORS ACROSS ALL SIMULATIONS - - -\n")
            out_file.write(f"(WHOLE) Average Queue Length: {sum_q_len_whole / num_sims:.4f}\n")
            out_file.write(f"(4 HR) Average Queue Length: {sum_q_len_4hr / num_sims:.4f}\n")
            out_file.write(f"(WHOLE) Average Queue Time: {sum_q_time_whole / num_sims:.4f}\n")
            out_file.write(f"(4 HR) Average Queue Time: {sum_q_time_4hr / num_sims:.4f}\n")
            out_file.write(f"(WHOLE) Average Vehicular Flow Rate: {sum_vfr_whole / num_sims:.2f}\n")
            out_file.write(f"(4 HR) Average Vehicular Flow Rate: {sum_vfr_4hr / num_sims:.2f}\n")
            out_file.write(f"Cars Serviced on Average: {sum_cars_serviced / num_sims:.2f}\n")

            # --- Write average interval results ---
            ...
            for i in range(16):
                ...
                start_hr = 10 + 0.25 * i # Start at 10AM
                end_hr = 10 + 0.25 * (i + 1)
